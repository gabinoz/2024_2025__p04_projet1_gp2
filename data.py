...
def verifier_binaire(nb):  #utilisation de chat gpt pour la fonction
    ...
    if all(char in '01' for char in nb): # vérifier que le nombre ne contient que des 0 et/ou des 1, si c'est vrai alors le nombre ets bien en base binaire
        print(f"L'utilisateur dit que le nombre est en base binaire : {nb}. C'est vrai, on peut continuer le code.")
    else: #si le nombre possède d'autres chiffres que 0 et 1 alors il n'est pas en base en binaire
        print(f"L'utilisateur dit que le nombre est en base binaire : {nb}. C'est faux, veuillez entrer un nombre dans la base binaire pour continuer le code.")
...
def verifier_decimal(nb): #fait sur le meme modèle sans chat gpt
    ...
    if all(char in '0123456789' for char in nb):
        print(f"L'utilisateur a dit que le nombre est en base décimale : {nb}. C'est vrai, on peut continuer le code.")
    else:
        print(f"L'utilisateur a dit que le nombre est en base décimale : {nb}. C'est faux, veuillez entrer un nombre dans la base binaire pour continuer le code.")
...
def verifier_hexa(nb):  #fait sur le meme modèle sans chat gpt
    ...
    if all(char in '0123456789abcdefABCDEF' for char in nb): # l'utilisteur peut utiliser des lettres minuscules ou majuscules, il faut donc le prendre en compte
        print(f"L'utilisateur a dit que le nombre est en base hexadécimale : {nb}. C'est vrai, on peut continuer le code.")
    else:
        print(f"L'utilisateur a dit que le nombre est en base hexadécimale : {nb}. C'est faux, veuillez entrer un nombre dans la base héxadécimal pour continuer le code.")
